...
\item[Define your own JOD shortcuts.]  JOD words can be used within arbitrary J programs.  If you don't find a JOD primitive that meets your needs do a little programming.
...
\item[Define JOD project macros.]  When programming with JOD you typically open dictionaries, load system scripts and define nouns. This can be done in a project macro script: see \hyperlink{il:rm}{\texttt{rm}} on page~\pageref{ss:rm}.
...
\end{verbatim}
is run: see appendix~\ref{ap:jodprofile} on page~\pageref{ap:jodprofile}.  Use this script to customize JOD. Note how you can execute project macros when JOD loads with sentences like:
...
\item[Use JOD help and documentation.]  JOD documentation is available in two forms.  A PDF document \verb|jod.pdf|  is installed with the addon. For online help use  \hyperlink{il:jodhelp}{\texttt{jodhelp}}: see page~\pageref{ss:jodhelp}.
