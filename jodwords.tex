...
\subsection{\texttt{abv} --- all backup versions}\label{ss:abv}\index{backup!\texttt{abv}} 
...
\subsection{\texttt{addgrp} --- add words/tests to group/suite}\label{ss:addgrp}\index{group!\texttt{addgrp}} 
...
\end{verbatim}
 ...
 \noindent is an example of a compressed J script. In its fully commented
  form this script is about 245 kilobytes when squeezed with
  \texttt{compj} it shrinks to about 86 kilobytes.
...
\end{enumerate}
    ...
    More examples of the use of comment tags can be found in 
     ...
     scripts at \jodurl{https://github.com/bakerjd99/jod/tree/master/jodijs}.
...
} Master file\index{master file} parameters\index{parameters|see{dictionary}} are initially 
...
you are absolutely sure you will not reset other tasks!\index{hash}
...
with a sidecar text file of SHA-256 hashes.\index{hash} Backup 13's sidecar file is \verb|13jhashes.txt|. If a \texttt{packd} operation succeeds, 
...
\hypertarget{il:restd}{\texttt{restd}}  restores  backups\index{backup!\texttt{restd}} created by
