...
on page~\pageref{ap:classes}), and defines a \texttt{\textbf{z}} 
locale\index{z interface} user interface.
JOD's \texttt{\textbf{z}} user interface consists of definitions like:  
...
\end{verbatim}
  ...
  is an example of a compressed J script. In it�s fully commented
  form this script is about 203 kilobytes when squeezed with
  \texttt{compj} it shrinks to about 69 kilobytes.
...
\end{lstlisting}
    ...
    More examples of the use of comment scope tags can be found in 
     ...
     addon or by downloading \jodsource  from 
     \href{http://bakerjd99.googlepages.com/home}{\emph{The JOD Pages}}.  
...
\begin{lstlisting}[frame=single,framerule=0pt]  
  ...
  NB. get group explaination text
...
\hypertarget{il:jodhelp}{\texttt{jodhelp}} displays help\index{help!\texttt{jodhelp}}
 for JOD words.  
...
If a group\index{group!\texttt{locgrp}} is named \texttt{numtutils} the associated postprocessor 
...
\subsection{\texttt{locgrp} --- list groups/suites with word/test}
...
\begin{verbatim}
  ~config\startup.ijs
\end{verbatim}
and inserted in the session's \verb|PUBLIC_j_| table.
...
\subsection{\texttt{nw} --- edit a new explicit word}\label{ss:nw} 

\hypertarget{il:nw}{\texttt{nw}} edits\index{edit!\texttt{nw}} a new explicit word in an edit window using JOD format conventions.  \texttt{nw} will append the character list \texttt{DOCUMENTCOMMAND} to the text placed in an edit window.  This allows the user to define an arbitrary script that is run when a word is defined with \texttt{CTRL-W}.
...
\texttt{nt} looks for the test script \texttt{TESTSTUB} on the path and inserts \texttt{TESTSTUB} text in the edit window.   \texttt{TESTSTUB} allows users to define dictionary specific
...
Because most updates are put�s the first dictionary is called the \emph{put dictionary}.
...
\subsection{\texttt{regd} --- register dictionaries}

\hypertarget{il:restd}{\texttt{restd}} registers\index{registration!\texttt{regd}} and unregisters 
